...
def create_case_ppt(analysis_json, folder):
    ...
    # Hashtags: clean, enforce 3 unique (WITHOUT adding #)
    hashtags = list(dict.fromkeys(data.get("hashtags") or []))[:3]
    cleaned_tags = []
    for tag in hashtags:
        tag = tag.replace("#", "").strip()
        tag = " ".join(tag.split()[:3])  # max 3 words
        if tag:
    ...
    # Business Categories: clean + dedupe
    bcs = data.get("business_categories") or []
    seen = set()
    clean_bcs = []
    for x in bcs:
        item = " ".join(x.split()[:2]).strip()  # max 2 words
    ...
    out_file = os.path.join(
        folder, f"BIP_MCG_Case Study_{data['case_study_name']}.pptx"
    )
    prs.save(out_file)
    return out_file
...
# Streamlit App
# -----------------------------
st.title("📊 BIP Case Study Processor")
...
if uploaded_files:
    for f in uploaded_files[:20]:  # hard cap at 20
        ...
        # ✅ Feedback
        st.success(f"Case study {case_name} processed!")
        st.write(f"- Saved folder: {folder}")
        st.write(f"- Files: raw.json, parsed.csv, Original - {f.name}, and {os.path.basename(out_ppt)}")
